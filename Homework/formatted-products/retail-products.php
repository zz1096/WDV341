...
try {
    ...
    $sql = "SELECT product_name, product_image, product_description, product_price, product_inStock, product_status FROM wdv341_products ORDER BY product_name DESC";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
    $result = $stmt->setFetchMode(PDO::FETCH_ASSOC); 
}
catch(PDOException $e){
    $message = "<h3 style='text-align: center'>There has been a problem. The system administrator has been contacted. Please try again later.</h3>";
    ...
    <section>
        ...
        <?php
            while($row = $stmt->fetch()){
                ...
                $productInventory = $row['product_inStock'];
        ?> 
            <div class="productBlock">
                ...
                ?>
                    <p class="productLowInventory"><?php echo $row['product_inStock']; ?> In Stock!</p>
                <?php
                    }
                    else{
                ?>
                    <p class="productInventory"><?php echo $row['product_inStock']; ?> In Stock!</p>
                <?php
                    }
                ?>
            </div>
